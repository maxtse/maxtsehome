...
public abstract class BaseGenericObjectPool<T> {
    ...
    private volatile EvictionPolicy<T> evictionPolicy;//驱逐检测策略
    ...
    private Evictor evictor = null; // @GuardedBy("evictionLock")//驱逐检测任务
    EvictionIterator evictionIterator = null; // @GuardedBy("evictionLock")//驱逐检测迭代器
    /*
    ...
    final void startEvictor(long delay) {
        synchronized (evictionLock) {
            ...
            if (delay > 0) {//只有设置了延迟时间，才会去执行，<=0的则不会
                evictor = new Evictor();
