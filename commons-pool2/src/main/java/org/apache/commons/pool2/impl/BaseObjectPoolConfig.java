...
public abstract class BaseObjectPoolConfig implements Cloneable {
     ...
     * myself 当borrowObject无可用对象时，最大阻塞时间，超过抛出异常，当为-1时，一直阻塞
     */
     ...
     * 空闲时是否校验有效性，用于evict()时，如果不需要驱逐，然后检测是否有效
     */
     ...
     * 小于0 不启动检测 startEvictor(long delay)
     */
    ...
    private boolean lifo = DEFAULT_LIFO;//后进先处理策略

    private boolean fairness = DEFAULT_FAIRNESS;//公平锁

    private long maxWaitMillis = DEFAULT_MAX_WAIT_MILLIS;//borrow时，无空闲对象，最大等待时间

    private long minEvictableIdleTimeMillis =
        DEFAULT_MIN_EVICTABLE_IDLE_TIME_MILLIS;//最小的空闲时间 超过将会驱逐

    private long softMinEvictableIdleTimeMillis =
            DEFAULT_MIN_EVICTABLE_IDLE_TIME_MILLIS;//软引用对象池的最小空闲时间

    private int numTestsPerEvictionRun =
        DEFAULT_NUM_TESTS_PER_EVICTION_RUN;//每次检测多少个

    private String evictionPolicyClassName = DEFAULT_EVICTION_POLICY_CLASS_NAME;//默认的驱逐策略的类名

    private boolean testOnCreate = DEFAULT_TEST_ON_CREATE;//创建后是否检测有效性

    private boolean testOnBorrow = DEFAULT_TEST_ON_BORROW;//创建后是否检测有效性

    private boolean testOnReturn = DEFAULT_TEST_ON_RETURN;//创建后是否检测有效性

    private boolean testWhileIdle = DEFAULT_TEST_WHILE_IDLE;//空闲时是否校验有效性，用于evict()时，如果不需要驱逐，然后检测是否有效

    private long timeBetweenEvictionRunsMillis =
        DEFAULT_TIME_BETWEEN_EVICTION_RUNS_MILLIS;//检测的时间间隔 小于0则不启动

    private boolean blockWhenExhausted = DEFAULT_BLOCK_WHEN_EXHAUSTED;//是否阻塞方式获取对象
